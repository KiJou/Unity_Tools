...
VertexOutput vert(VertexInput v)
{
    ...
    float Set_Outline_Width = (BaseWidth * 
        //smoothstep(_Farthest_Distance, _Nearest_Distance, distance(objPos.rgb, _WorldSpaceCameraPos)) *
        smoothstep(far, near, distance(objPos.rgb, _WorldSpaceCameraPos)) *
    ...
    //v.2.0.4.2
        ...
        float diff = clamp(max(0, dot(normalize(v.normal), eyeDir)), 0, 1);
        _Offset_Z = _Offset_Z * -0.01;
    #endif
    ...
    //v2.0.4
        ...
        //o.pos.xy = o.pos.xy + offset * Set_Outline_Width;
        float4 a = float4(v.vertex.xyz + normalize(v.normal) * Set_Outline_Width, 1);
        float4 b = float4(v.vertex.xyz + _BakedNormalDir * Set_Outline_Width,1);
        float4 value = lerp(a, b, _Is_BakedNormal);
...
float4 frag(VertexOutput i) : SV_Target
{
    ...
    float3 _OutlineTex_var = tex2D(_OutlineTex, TRANSFORM_TEX(Set_UV0, _OutlineTex));
        ...
        Set_Outline_Color = lerp(
            float4(_Is_BlendBaseColor_var, 1.0), 
            float4(_OutlineTex_var.rgb*_Is_BlendBaseColor_var, 1.0), 
            _Is_OutlineTex);
